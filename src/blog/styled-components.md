---
title: Styled Components без боли
date: '2021-01-19'
tags:
  - css
  - cssinjs
...
Сразу задам тон повествования: я предполагаю, что с JavaScript и React вы уже знакомы и мне не придётся убеждать кого-либо в том, что JSX есть хорошо, а компонентный подход – удобно. Я себя-то два года убеждал, разве что отдельно попросите. Вам сейчас придётся не только принять, что HTML в JS это хорошо, но и что CSS в JS, как минимум, не хуже.
...
- Отсутствие "мёртвого" (неиспользуемого) кода
...
- Композиция классов становится сложнее, чем должна бы
...
function HelloVarComponent() {
  return (
    <article>
      {colors.map(color => {
        return (
          <div className="hello" style={{"--color": color}}>
